...
generally, a matrix in which the entries are drawn from independently
and identically distributed (\textit{iid}) distributions. For example, one of the simplest
...
For a system as complex as the human brain, RMT is thus tantalizing. If some
matrix representation of brain activity can be described, even partially, by
aspects of RMT theory, this would mean  description of this matrix either aligned nicely with
RMT theory, or otherwise was broadly predictively useful.
...
\cite{guhrRandommatrixTheoriesQuantum1998a}. Viewing the unfolded eigenvalues
as an \textit{irregular} timeseries, the level number varaince is simply the
variance of the number of samples of all intervals of length \(L\) over the
series.

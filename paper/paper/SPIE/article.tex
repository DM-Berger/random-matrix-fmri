...
\citenum{wangRandomMatrixTheory2016,guRandomMatrixTheory2020} use a
Kolmogorov-Smirnov test, and \citenum{wangSpectralPropertiesTemporal2015a} uses
a t-test to provide evidence of differences in various RMT metrics. However,
statistically significant differences in a metric do not imply practical
significance or predictive utility \cite{loWhySignificantVariables2015}.
...
is computationally-demanding and non-trivial, requiring an \textit{unfolding}
...
making a number of arguably subjective decisions. These fitting decisions can
...
with the absence of clear code, and exclusively descriptive/explanatory
analytic approaches means that the practical utility of RMT for fMRI is largely
uninvestigated.
...
non-depressed controls and mildy- or moderately-depressed subjects. In our analysis,
...
\code{task\_attend v tas\_nonattend} classification task was constructed
...
measurements) was missing, but also because all pipelined intermediates were saved to later
...
values, extreme values are often trimmed (omitted from fitting)
\cite{moralesImprovedUnfoldingDetrending2011,
abueleninEffectUnfoldingSpectral2012}. The number of eigenvalues to trim is
highly subjective, and not usually well-documented, and unfortunately, with a
large number of different source matrices, it is too destructive to naively use
the same hard criterion (e.g. largest 10\%, largest 3) for all unfoldings.
...
of 3, 5, 7, and 9 to yield 8 total ``smoothed eigenvalue'' features.

\subsubsection{Feature Combinations}

A combination of RMT and non-RMT eigenfeatures
could be more predictive than either alone. We test this with  a variety of eigenfeature
combinations. The particular combinations are especially chosen to
test whether RMT eigenfeatures add anything beyond smoothing.

Combined features are formed by concatenation, so that if we have features
\(f_1, \dots,  f_n\) with dimensions \(p_1, \dots, p_n\), then the combined
feature is \([f_1; \dots; f_n]\) with \(\sum p_i\) dimensions.  The final
combinations chosen can be found in Table \ref{tab:features}.


\subsubsection{Slicing Variants}

The largest, middle, or smallest eigenvalues could be most useful
in characterizing some system. E.g. if the smallest eigenvalues correspond to
random / noise aspects of a system, but differences in the nature of the
system noise most differentiate between systems, then the smallest
...
percentile). These statistics are used to reduce the fMRI data
...
feature has a uniformly smoothed variant, where the size of the smoothing
...
9 (KNN3, KNN5, and KNN9, respectively). In all cases the default hyperparameter
values were used. We originally also attempted to test a simple logistic
...
strongly separated (e.g. Cohen’s \(d > 1.0\); \citenum{dansereauStatisticalPowerPrediction2017}).
In this study eigenfeatures could improve upon guessing by 16.2\% - 33.7\% (\ref{S-tab:acc-numerical}).

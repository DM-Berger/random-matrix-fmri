...
\affil[e]{Nova Scotia Health Authority, Research Affiliate, Nova Scotia, Canada}
...
Though first developed to describe the fluctuation of nuclei energy levels in
quantum physics \cite{mehtaRandomMatrices2004,
guhrRandommatrixTheoriesQuantum1998a}, Random Matrix Theory (RMT) has been
shown to have extremely broad potential. In small-scale physical systems, RMT
universalities have been observed in quantum chaotic systems, complex nuclei,
atoms, molecules and disordered mesoscopic systems
...
eigenvalues—also often called \textcolor{orange}{the \textit{spectra} or}
\textit{levels}—of a number of classes of random
...
a matrix where each entry is a random variable. Typically, each random variable
is \textcolor{orange}{independent} and identically distributed (\textit{iid}), however, modern
extensions allow for some
dependence\cite{bouchaudFinancialApplicationsRandom2009}. A \textit{class} or
\textit{ensemble} of random matrices can be defined by the type of
\textit{iid} distribution involved. For example, the Gaussian Orthogonal
Ensemble (GOE\cite{mehtaRandomMatrices2004,
guhrRandommatrixTheoriesQuantum1998a}) is a \textit{class} or \textit{ensemble}
of random matrices that comprises orthogonal matrices with each entry being
sampled from a standard Gaussian, and the Marchenko-Pastur distribution
describes the limiting behaviour of the singular values of \textit{iid}
rectangular random matrices where the \textit{iid} distribution is
arbitrary\cite{mehtaRandomMatrices2004}. RMT often finds that in the infinite
limit, the expected eigenvalue distribution of a number of classes of random
matrices can be described quite
precisely\cite{andersonIntroductionRandomMatrices2010,
akemannOxfordHandbookRandom2011, mehtaRandomMatrices2004}.

Of course, given a particular non-random matrix or instantiation of a random
matrix, one can, in general, only estimate the likelihood that it is a member
of a certain class or ensemble, since certain RMT ensembles (such as the GOE)
can yield arbitrary matrices with extremely low probability. Thus, RMT also
includes statistical measures ("spectral observables") and tools to help
compare empirical observations to theory\cite{mehtaRandomMatrices2004,
guhrRandommatrixTheoriesQuantum1998a}.
...
% well-described by the statistical behaviour of those eigenvalues.
...
statistical regularities to reliably surface}. If such a system has a matrix
representation, and the eigenvalues \textcolor{orange}{and spectral
observables} of this representation have distributions similar to
\textcolor{orange}{those} predicted by RMT, it suggests the system is either
highly random or chaotic. By contrast, if the observed spectra deviate
significantly from RMT-predicted spectra, this suggests otherwise. A number of
studies have used RMT to make such interpretations and comparisons between
...
\cite{matharooSpontaneousBackpainAlters2020}, and between left-sided
vs.\ right-sided motor imagery \cite{guRandomMatrixTheory2020}. While no differences
...
be useful for feature extraction. However, different feature
...
requirements} for calculating the full \(N \times N\) matrix \(\mathbf{M}\) of
...
performed first on each feature to be combined. \textcolor{orange}{For example}
if we have features \(f_1, \dots,  f_n\) with dimensions \(p_1, \dots, p_n\),
then the sliced max 25\% feature for each component feature, \(\hat{f}_i\) is
the last \(p_i / 4\) elements of \(f_i\), and the combined sliced feature is
\([\hat{f}_1;\; \dots;\; \hat{f}_n]\).
% \footnote{Exact implementation can be found in the
%\href{https://github.com/DM-Berger/random-matrix-fmri/blob/7c9e4187f582dedee728cd7193b8894d928c2f00/code/rmt/updated\_predict.py\#L182-L213}{source
%code}}.
...
\textcolor{orange}{Because the computation of these statistics is
...
\textcolor{cyan}{Given the sensitivity to these decisions observed in this
study, and considering the lack of consensus regarding the preprocessing of
fMRI or other complex biological signal data, RMT should probably not be
currently considered a tool with "out-of-the-box" utility in these domains.
While RMT likely still has potential to yield insights in a variety of
contexts, our results suggest that, absent strong evidence otherwise, it is not
necessarily safe to assume that these insights will generalize broadly beyond
the specific preprocessing and analytic pipelines involved.}

\color{black}

\textcolor{cyan}{Further research might establish specific sets of analytic
choices that allow RMT to consistently extract useful information in a wide
variety of contexts. However, it is also possible that each unique context
might require a specific combination of choices. In this latter case, there
should be \textit{strong} theoretical justification for the use of
RMT, and for each of the various analytic choices involved in its use: the
variability of results seen in this study suggests that there may often be a
set of analytic choices that provide favourable results, and which can be
weakly justified by plausible, but ultimately \textit{post hoc} justifications.}
\textcolor{cyan}{Provided that these precautions are followed, and that} future
studies employing RMT \textcolor{orange}{also} carefully investigate the
sensitivity of any findings to such analytic decisions, \textcolor{cyan}{then
there is likely considerable untapped potential for RMT in the analysis of fMRI.}

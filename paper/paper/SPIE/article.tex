...
\citenum{wangRandomMatrixTheory2016,matharooSpontaneousBackpainAlters2020}, we have also elected to use
...
The PSYCH Dataset  is detailed in \citenum{gorgolewskiHighResolution7Tesla2015}. Subjects completed a
...
The PANAS-X \citenum{watsonPANASXManualPositive1994} is a 9-point Likert-type self-report measure that
...
\citenum{shalevConjunctiveContinuousPerformance2011} is a behavioural measure of sustained attention
...
from German; \citenum{gorgolewskiCorrespondenceIndividualDifferences2014}. One question assesses
...
\citenum{tetreaultBrainConnectivityPredicts2016}. Subjects included healthy controls (``nopain''
...
complementary to \citenum{matharooSpontaneousBackpainAlters2020}.

\subsubsection{Dataset 3}
Dataset 3  (referred to here with the label ``REFLECT'', due to the ``reflective” nature of the
cognitive tasks) is described in full detail by \citenum{dupreMultiechoFMRIReplication2016}. Both
multi-echo rs- and task-fMRI scans were conducted with scan parameters summarized in Table \ref{table:2}. Tasks
involved viewing various affective images (see \citenum{sprengPatternsBrainActivity2010} for details)
...
image with overall increased contrast \citenum{kunduMultiechoFMRIReview2017}. In the ``interleave''
...
described in detail in \citenum{madhyasthaDynamicConnectivityRest2015}. Scan parameters are documented
...
(ANT; \citenum{fanActivationAttentionalNetworks2005} ). Subgroup details are available in Table \ref{table:1}. As the ANT
...
detail in \citenum{schapiroHumanHippocampalReplay2018}. In this study, both rs-fMRI and task fMRI scans
...
related by family-resemblance of the components see figure 1 of \citenum{schapiroHumanHippocampalReplay2018}. After this learning task, subjects were tested on their
...
\caption{
    ...
    Task = fMRI task. ANT = Attention Network Task \citenum{fanActivationAttentionalNetworks2005}
}
\label{table:2}
\small
\centering

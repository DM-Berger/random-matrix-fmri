...
interprting past and future studies applying RMT.
...
Random Matrix Theory (RMT) has obvious potential.  In small-scale physical
...
\cite{raiRandomnessPreservedPatterns2015}.
...
independently and identically distributed (\textit{iid}) distributions.
...
RMT may thus have real potential in understanding the human brain. For an
imaging modality like functional magnetic resonance imaging (fMRI), where
changes in the blood-oxygenation-level-dependent (BOLD) signals are related to
neural activity, RMT may be an ideal starting point. Each voxel or collection
of voxels—i.e. Region of Interest (ROI\footnote{We will use ROI to refer to
\textit{any} collection of voxels of any size. E.g. a single voxel is still an
ROI.}); any collection )—can be considered an interacting component of the system.

Likewise, in \textbf{functional connectivity} analyses, statistical
relationships of the BOLD ROI time-courses are investigated in the hope of
gaining insights into brain functioniong (see
...
investigated in this study. Because the computation of these statistics is
non-trivial, we developed and release the separate, open-source Python library
...
procedures to the appendix, or refer readers to the source code [link].
...
It is possible that only some eigenvalues
contain predictively-useful information. Broadly, we might expect the largest,
middle, or smallest eigenvalues to be most useful in characterizing some
system. E.g. if the smallest eigenvalues correspond to random / noise aspects
of a system, but it the differences in the nature of the noise that most
differentiate between certain states, then here the smallest eigenvalues may
have the most predictive utility.

Likewise, the \(L\) value in each RMT eigenfeature defines the degree of
locality at which we summarize the spectrum. E.g. at small \(L\)-values, the
spectral rigidity summarizes the non-linearity of eigenvalues thar are
relatively close to each other. At large values of \(L\), the rigidity
summarizes the long-range non-linearity of the spectrum. E.g. larger \(L\)
values summarize the spectrum more globally, and smaller \(L\) values summarize
it more locally.

Since predictive utility may vary with summary locality, or with the region of
the original spectrum, we investigate various contiguous front, middle, and end
slices of each eigenfeature in all analyses, and where the size of each slice
is either 5\%, 10\%, or 20\% of the full eigenfeatures for the front and end.
silces, and 10\%, 20\%, and 40\% for the middle eigenvalues.

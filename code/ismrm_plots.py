...
ISMRM_FOLDER = Path.home() / "Documents/ISMRM"
...
def make_stacked_accuracy_histograms(
    ...
    def hist_over_trim(trim: str, unfold=UNFOLDS, fullpre=fullpre, normalize=normalize, fmt=fmt):
        ...
        for i, (hist_data, bins, guess, title) in enumerate(zip(hist_info, bins_all, guesses, titles)):
            ...
            ax.set_title(title, fontdict={"fontsize": FONTSIZE, "fontname": "CMU Serif"})
        ...
        text = dict(ha="center", va="center", fontsize=FONTSIZE)
        fig.legend(handles, labels, loc=[0.87, 0.10], fontsize=FONTSIZE, labelspacing=0.4)
        fig.text(0.5, 0.02, "Prediction Accuracy Using RMT Feature", **text)  # xlabel
        ...
        fig.subplots_adjust(top=0.905, bottom=0.105, left=0.065, right=0.975, hspace=0.8, wspace=0.35)
...
def plot_pred_rigidity(
    ...
    for trim_idx in ["(1,-1)"]:
        ...
        for i, pair in enumerate(all_pairs):
            ...
            sbn.lineplot(x=df1.index, y=boots1["mean"], color="#0066B3", label=g1, ax=ax)
            ax.fill_between(x=df1.index, y1=boots1["low"], y2=boots1["high"], color="#0066B3", alpha=0.3)
            if ensembles:
                L = df1.index
                poisson = GDE.spectral_rigidity(L=L)
                goe = GOE.spectral_rigidity(L=L)
                sbn.lineplot(x=L, y=poisson, color="#b0b0b0", label="Poisson", ax=ax)
                sbn.lineplot(x=L, y=goe, color="#0066FF", label="GOE", ax=ax)
            ax.legend().set_visible(False)
            ax.set_title(f"Unfolding Degree {unfold[i]}", fontname="CMU Serif", fontsize=HALFSIZE)
            ax.set_xlabel("")
            ax.set_ylabel("")
        axes.flat[0].legend(fontsize=12, labelspacing=0.2, framealpha=0.5).set_visible(True)
        text = dict(ha="center", va="center", fontsize=FONTSIZE)
        fig.text(0.5, 0.04, "L", **text)  # xlabel
        # fig.text(0.05, 0.5, "∆₃(L)", rotation="vertical", fontdict={"fontname": "DejaVu Sans"}, **text)  # ylabel
        fig.text(
            0.02,
            0.5,
            r"$\Delta _3 \left( L \right)$",
            rotation="vertical",
            # fontdict={"fontname": "DejaVu Sans"},
            fontdict={"fontname": "CMU Serif"},
            **text,
        )  # ylabel
        fig.set_size_inches(w=HALFWIDTH, h=2.36)
    ...
    for trim_idx in ["(1,-1)"]:
        ...
        for i, pair in enumerate(all_pairs):
            ...
            if ensembles:
                ...
                ax.plot(L, goe, "--", color="#0066B3", label="GOE")
            ax.legend().set_visible(False)
            ax.set_title(f"Unfolding Degree {unfold[i]}", fontname="CMU Serif", fontsize=HALFSIZE)
            ax.set_xlabel("")
            ax.set_ylabel("")
...
if __name__ == "__main__":
    ...
    # make_stacked_accuracy_histograms(ARGS, trim="1", fignum="0", fmt="png")
    # make_stacked_accuracy_histograms(ARGS, trim="20", fignum="s1", fmt=["png", "svg"])
    # plot_pred_rigidity(ARGS, "OSTEO", "duloxetine_v_nopain", show=False, fignum="1", fmt="svg")
    # plot_pred_levelvar(ARGS, "OSTEO", "duloxetine_v_nopain", ensembles=False, show=False, fignum="2", fmt="svg")
    plot_pred_rigidity(ARGS, "OSTEO", "duloxetine_v_nopain", ensembles=False, show=False, fignum="1", fmt="png")
    plot_pred_levelvar(ARGS, "OSTEO", "duloxetine_v_nopain", ensembles=False, show=False, fignum="2", fmt="png")

...
def make_stacked_accuracy_histograms(
    ...
    def hist_over_trim(
        ...
        fig.subplots_adjust(
            top=0.905, bottom=0.105, left=0.065, right=0.975, hspace=0.8, wspace=0.35
        )
...
def plot_pred_rigidity(
    ...
    for trim_idx in ["(1,-1)"]:
        ...
        for i, pair in enumerate(all_pairs):
            ...
            ax.set_title(
                f"Unfolding Degree {unfold[i]}", fontname="CMU Serif", fontsize=HALFSIZE
            )
            ax.set_xlabel("")
            ax.set_ylabel("")
        axes.flat[0].legend(fontsize=12, labelspacing=0.2, framealpha=0.5).set_visible(
            True
        )
        text = dict(ha="center", va="center", fontsize=FONTSIZE)
